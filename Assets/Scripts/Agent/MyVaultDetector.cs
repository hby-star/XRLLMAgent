...
public class MyVaultDetector : MonoBehaviour
{
    ...
    [SerializeField] private float m_minAdvance = 0.1f; //The minimum advance required to trigger a vault.
    ...
    public void Awake()
    {
        ...
        {
            Debug.LogError(
                "VaultDetector: Trying to Awake Vault Detector with null or empty vault configurations (m_vaultConfiguration)");
            Destroy(this);
            return;
        }
        ...
        {
            Debug.LogError(
                "VaultDetector: Trying to Awake Vault Detector with null or empty vault definitions (m_vaultDefinitions)");
            Destroy(this);
            return;
        }
        ...
        {
            Debug.LogError(
                "VaultDetector: Trying to Awake Vault Detector but the MxMAnimator component cannot be found");
            Destroy(this);
            return;
        }
        ...
        {
            Debug.LogError(
                "VaultDetector: Trying to Awake Vault Detector but there is no Trajectory component found that implements IMxMTrajectory.");
            Destroy(this);
            return;
        }
    ...
    public void Update()
    {
        ...
        if (Physics.SphereCast(probeRay, m_curConfig.DetectProbeRadius, out RaycastHit probeHit,
                m_maxVaultRise + m_maxVaultDrop,
                m_layerMask, QueryTriggerInteraction.Ignore))
        {
            ...
            if (probeHit.distance < (m_maxVaultRise - m_minVaultRise))
            {
                ...
                {
                    case EVaultContactOffsetMethod.Offset:
                    {
                        vaultable.Contact1 += transform.TransformVector(vaultDef.Offset_Contact1);
                    }
                        break;
                    case EVaultContactOffsetMethod.DepthProportion:
                    {
                        vaultable.Contact1 += transform.TransformVector(vaultable.Depth * vaultDef.Offset_Contact1);
                    }
                        break;
                }
                ...
                if (vaultable.VaultType == EVaultType.StepOver)
                {
                    switch (vaultDef.OffsetMethod_Contact2)
                    {
                        case EVaultContactOffsetMethod.Offset:
                        {
                            vaultable.Contact2 += transform.TransformVector(vaultDef.Offset_Contact2);
                        }
                            break;
                        case EVaultContactOffsetMethod.DepthProportion:
                        {
                            vaultable.Contact2 += transform.TransformVector(vaultable.Depth * vaultDef.Offset_Contact2);
                        }
                            break;
                    }
    ...
    private void VaultOffShapeAnalysis(in RaycastHit a_rayHit, out VaultableProfile a_vaultProfile, float hitOffset)
    {
        ...
        for (int i = 1; i < m_vaultAnalysisIterations; ++i)
        {
            ...
            if (Physics.Raycast(ray, out rayHit, m_maxVaultRise + m_maxVaultDrop, m_layerMask,
    ...
    *********************************************************************************************/
    private void VaultShapeAnalysis(in RaycastHit a_rayHit, out VaultableProfile a_vaultProfile)
    {
        ...
        for (int i = 1; i < m_vaultAnalysisIterations; ++i)
        {
            ...
            if (Physics.Raycast(ray, out RaycastHit rayHit, m_maxVaultRise + m_maxVaultDrop, m_layerMask,
                    QueryTriggerInteraction.Ignore))
            {
